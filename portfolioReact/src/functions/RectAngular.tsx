export function RectAngular(
  ...
  rect.stop = function () {
    ...
    if (highscore.length < 5 || rect.draw.score > highscore[4].score) {
      ...
      highscore.push({
        name: name,
        score: rect.draw.score,
        date: rect.draw.start,
      });
  ...
  rect.area.onclick = function (e) {
    ...
    if (rect.draw.start !== null) {
      ...
      if (rect.draw.left > 0) {
        let x =
          e.pageX - (rect.area.getBoundingClientRect().left + window.scrollX);
        let y =
          e.pageY - (rect.area.getBoundingClientRect().top + window.scrollY);

        /* Step One: Set x1 and y1 */
        if (
          (!rect.draw.x1 && !rect.draw.y1 && !rect.draw.x1 && !rect.draw.y1) ||
          (rect.draw.x1 && rect.draw.y1 && rect.draw.x2 && rect.draw.y2)
        ) {
          rect.draw.x1 = x;
          rect.draw.y1 = y;
          rect.draw.x2 = null;
  ...
  rect.paint = function () {
    ...
    /* Update Score */
    rect.draw.score = rect.draw.start
      ? Math.abs(new Date().getTime() - rect.draw.start.getTime())
      : 0;

    /* Display Line Left */
    rect.ctxt.fillText(
      'Line: ' + (rect.draw.left > 0 ? rect.draw.left : 0),
      rect.area.width - 190,
      35
    );
    ...
    if (rect.draw.tx && rect.draw.ty) {
      if (!rect.draw.x1 && !rect.draw.y1 && !rect.draw.x1 && !rect.draw.y1) {
        ...
        rect.ctxt.strokeRect(
          rect.draw.x1,
          rect.draw.y1,
          -(rect.draw.x1 - rect.draw.tx),
          -(rect.draw.y1 - rect.draw.ty)
        );
        ...
        rect.ctxt.fillRect(
          rect.draw.x1,
          rect.draw.y1,
          -(rect.draw.x1 - rect.draw.x2),
          -(rect.draw.y1 - rect.draw.y2)
        );
      }
    } else {
      /* Draw Rect (Touch) */
    ...
    if (
      rect.ball.x < 0 ||
      rect.ball.y < 0 ||
      rect.ball.x > rect.area.width ||
      rect.ball.y > rect.area.height
    ) {
      rect.stop();
    }
